...
{
    [SerializeField]
    private float DistanceToWegren, RaycastEnemy;
    ...
    void RunAway()
    {
        ...
        else if (!Physics.Raycast(transform.position, transform.TransformDirection(Vector3.right), out hit, RaycastEnemy, layerMask))
        {
            Debug.DrawRay(transform.position, transform.TransformDirection(Vector3.right) * hit.distance, Color.yellow);
            Debug.Log("Did Hit right");
            transform.Translate(Vector3.right* Wegrenspeed * Time.deltaTime);
        }
        else if (!Physics.Raycast(transform.position, transform.TransformDirection(Vector3.left), out hit, RaycastEnemy, layerMask))
        {
            Debug.DrawRay(transform.position, transform.TransformDirection(Vector3.left) * hit.distance, Color.yellow);
            Debug.Log("Did Hit left");
            transform.Translate(Vector3.left * Wegrenspeed * Time.deltaTime);
        }
        //Diagonale raycast (Vector3(1,1,0))
        else
        {
